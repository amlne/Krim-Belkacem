...
public class FormAjoutPlanning {
	private boolean valide = false;
	private String erreur;
	public static final int TAILLE_TAMPON = 10240;
	public static final String CHEMIN_FICHIERS = "/home/fateh/projetLicenceSauv/planning/";

	private PlanningDao planningDao;
	private beans.Admin admin;

	public FormAjoutPlanning(beans.Admin admin) {
	...
	public void controleform(HttpServletRequest request) throws IllegalStateException, IOException, ServletException {
		...
		if (trimestre != null && classe != null && nomFichier != null) {
			if (!trimestre.isEmpty() && !classe.isEmpty() && !nomFichier.isEmpty()) {
				...
				int trim = 0;
				planning.getClasse().setId_cls(classe);
				if (trimestre.equals("1")) {
					planning.setTrimestre1(chemin_fichier + nomFichier);
					trim = 1;
				}
				if (trimestre.equals("2")) {
					planning.setTrimestre2(chemin_fichier + nomFichier);
					trim = 2;
				}
				if (trimestre.equals("3")) {
					planning.setTrimestre3(chemin_fichier + nomFichier);
					trim = 3;
				}
				if (trimestre.equals("annuel")) {
					planning.setAnnuel(chemin_fichier + nomFichier);
					trim = 4;
				}
				if (planningDao.ajoutPlanning(planning,trim) == true) {
					// *****************
